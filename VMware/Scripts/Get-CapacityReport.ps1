...
# Updated By: Sean Davis
# Based On Original Script From Marc Vincent Davoli
#
...
$SMTPserver = "SMTPSERVERGOESHERE" #Configure Server SMTP Relay Server
$LogFile = "C:\scripts\custom_logs\$ScriptName.log"
###################################################
#endregion
...
###################################################
$vCenterServer = "msc-vcenter01.man.co" #Defines The vCenter Being Reported On 
$ToAddress = "admins@yourdomain.com"
###################################################
#endregion
...
Function BuildESXiSoftwareAndHardwareInfoTable ($VMHostsTemp)
{
	...
	$ESXiSoftwareAndHardwareInfoTemp += "<tr><td width=200><b>Host Name</b></td><td width=200><b>Server Model</b></td><td width=130><b>Number of CPUs</b></td>" + "<td width=180><b>RAM Quantity (in GB)</b></td><td width=170><b>ESXi Version</b></td><td width=80><b>Uptime</b></td></tr>"
	...
	$ESXiSoftwareAndHardwareInfoTemp +=	$VMHostsTemp | Sort-Object name | ForEach-Object {
						"<tr><td>" + $_.Name + "</td><td>" + $_.Manufacturer + " " + $_.Model + "</td><td>" + $_.NumCpu + " CPUs </td><td>" + (GetTotalMemoryInGB ($_))	+ " GB </td><td>" + $_.Version + " Build " + $_.Build + "</td><td>" + ($_ | Get-View | select @{N="Uptime"; E={(Get-Date) - $_.Summary.Runtime.BootTime}}).Uptime.Days + " days </td>"
						}
...
# Builds CPU HTML Table and populates Arrays used to create chart
Function CreateVMHostCPUUsageTable ($VMHostsTemp)
{ 
	$CPUTableTemp = "<table border=1>"
	$CPUTableTemp += "<tr><th colspan=4><b><center> ESXi Host CPU Usage Statistics </center></b></th></tr>"
	$CPUTableTemp += "<tr><td width=200><b>Host Name</b></td><td width=130><b>Number of CPUs</b></td><td width=200><b>Current CPU Usage %</b></td>" + "<td width=250><b>Monthly Average CPU Usage %</b></td></tr>"
...
# Builds Memory HTML Table and populates Arrays used to create chart
Function CreateVMHostMemoryUsageTable ($VMHostsTemp)
{ 
...
###################################################
$ScriptVersion = "v2.2 - By Sean Davis" # Included in the Runtime info in $HTMLFooter
$Subject = "VMware Capacity & Performance Report for " + $vCenterServer
$FromAddress = $vCenterServer + "@man.co"

$ColorArray = "Red", "Orange", "Purple", "Blue", "Olive", "SlateGrey", "Orange", "Purple", "Blue", "Olive"
$ColorArrayIndex = 0

$HTMLHeader = "<HTML><TITLE> VMware Capacity & Performance Report for " + $vCenterServer + "</TITLE>"
$HTMLFooter = "Script Version " + $ScriptVersion + " running on server " + $env:COMPUTERNAME + "</HTML>"
###################################################
#endregion
...
#Add The PowerCLI Powershell Snap-In
If((Get-PSSnapin -Name VMware.VimAutomation.Core -ErrorAction SilentlyContinue) -eq $null)
{
	Add-PSSnapin VMware.VimAutomation.Core
}

#Disconnect Any Open vCenter Sessions
Try{Disconnect-VIServer * -Confirm:$false -ErrorAction SilentlyContinue} Catch{$null}

#Connect To vCenter Server
Try {
	Connect-VIServer $vCenterServer -WarningAction SilentlyContinue | Out-Null
...
#Gather vCenter Info
$DC = Get-Datacenter | Sort-Object -Property Name 
...
$Date = Get-Date | Sort-Object -Property Name
...
$HTMLBody = "<BODY>"
$HTMLBody += "$Date<br>"
$HTMLBody += "<h1><b><center>VMWare Capacity & Performance Report for " + $vCenterServer + "</center></b></h1>"

$HTMLBody += CreateHeader ("INVENTORY FOR <font color=green><b> vCENTER " + $vCenterServer + "</font></b>")
...
# HEADER
$HTMLBody += CreateHeader ("CPU CAPACITY REPORT FOR <font color=green><b> vCENTER " + $vCenterServer + "</font></b>")
...
# HEADER
$HTMLBody += CreateHeader ("MEMORY CAPACITY REPORT FOR <font color=green><b> vCENTER " + $vCenterServer + "</font></b>")
...
#Gather Datastore Stats
Write-Host Step 4/6 - Collecting Datastore information...

# HEADER
$HTMLBody += CreateHeader ("DATASTORE CAPACITY REPORT FOR <font color=green><b> vCENTER " + $vCenterServer + "</font></b>")
...
# HEADER
$HTMLBody += CreateHeader ("VIRTUAL MACHINE REPORT FOR <font color=green><b> vCENTER " + $vCenterServer + "</font></b>")
...
ForEach ($ClusterTemp in ($Clusters)){
	...
	If ($NumberOfVMHostsInCluster -gt 1){ #Ignore Clusters with no ESXi hosts in it
		...
		# INTRO TEXT
		$HTMLBody += "<b>" + [String]$VMHostsInCluster.Count + " ESXi Hosts with a total of " + (GetTotalCPUCyclesInGhz ($VMHostsInCluster)) + " GHz on " +	(GetTotalNumberofCPUs ($VMHostsInCluster)) + " CPUs and " + (GetTotalMemoryInGB ($VMHostsInCluster)) + " GB of RAM </b><br>"
		$HTMLBody += "<b>" + [String]$DatastoresInCluster.Count + " Datastores with a total of " + (GetTotalDatastoreDiskSpaceinGB ($DatastoresInCluster)) + " GB of disk space</b><br><br>"
